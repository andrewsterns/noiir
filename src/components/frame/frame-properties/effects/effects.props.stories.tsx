...
export const Effects: EffectsStory = {
  ...
  render: (args: EffectsArgs) => {
    const activeEffects = [];
    if (args.dropShadowEnabled) activeEffects.push('Drop Shadow');
    if (args.innerShadowEnabled) activeEffects.push('Inner Shadow');
    if (args.layerBlurEnabled) activeEffects.push('Layer Blur');
    if (args.backgroundBlurEnabled) activeEffects.push('Background Blur');
    if (args.noiseEnabled) activeEffects.push('Noise');
    if (args.textureEnabled) activeEffects.push('Texture');

    // Build effects object based on enabled effects
    const effectsProps: any = {};
