...
export interface FrameStateProps extends Omit<FrameProps, 'id' | 'variant' | 'variants' | 'children' | 'className' | 'style' | 'onClick' | 'onMouseEnter' | 'onMouseLeave'> {
...
export interface FrameVariants {
  [variant: string]: FrameStateProps;
}

// Helper to get props for a variant
// Returns merged props for a variant, falling back to 'default' if needed
export function getStateProps(variants: FrameVariants, variant: FrameVariantName, baseProps: FrameStateProps = {}): FrameStateProps {
  const stateProps = variants[variant] || variants['default'] || {};
...
export function createComponentVariants(baseVariants: FrameVariants, overrides: Record<string, Partial<FrameStateProps>> = {}): FrameVariants {
